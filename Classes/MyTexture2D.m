...
- (void) drawFromAtlasAtPointv2:(CGPoint)point key:(NSString*)key {
    ...
    if (value) {
        CGRect atlasRect = [value CGRectValue];
		[self drawInRect:CGRectMake(point.x - 80 / 2, point.y - 30 / 2, 80, 30) textureRect:atlasRect];

    }
}

- (void) drawFromAtlasAtPointFlip:(CGPoint)point key:(NSString*)key {
    NSValue* value = [_textureAtlas valueForKey:key];
    if (value) {
        CGRect atlasRect = [value CGRectValue];
        [self drawInRectFlip:CGRectMake(point.x - atlasRect.size.width / 2, point.y - atlasRect.size.height / 2, atlasRect.size.width, atlasRect.size.height) textureRect:atlasRect];
    }
}

- (void) drawAtPointFlip:(CGPoint)point {
  
	[self drawInRectFlip:CGRectMake(point.x - self.imageSize.width / 2, point.y - self.imageSize.height / 2, self.imageSize.width, self.imageSize.height) textureRect:CGRectMake(0, 0,self.imageSize.width,self.imageSize.height)];
...
- (void) drawInRectFlip:(CGRect)rect textureRect:(CGRect)textureRect {
    ...
    GLfloat	coordinates[] = {
        x + (self.maxS * (textureRect.size.width / self.imageSize.width)), 
		 y + (self.maxT * (textureRect.size.height / self.imageSize.height)),
        x, 
		 y + (self.maxT * (textureRect.size.height / self.imageSize.height)),
    ...
    if (content == nil) {
        ...
        for (NSString* line in lines) {
            ...
            [_textureAtlas setValue:[NSValue valueWithCGRect:rect] forKey:[data objectAtIndex:0]];
			  ...
			  if ([data count] == 6)
			  {
				  [_textureAtlas setValue:[data objectAtIndex:5] forKey:@"type"];
			  }
        }
    }
}
...
-(bool) animateonceflip:(int) x: (int) y
{
	...
	int index=0;
	for (id mykey in (_textureAtlas))
	{
		if (index == animestate)
		{
			[self drawFromAtlasAtPointFlip:CGPointMake(x, y) key: mykey];
	...
	if (delaycount >= delay) 
	{
		if (animestate != [_textureAtlas count]) animestate++;
		delaycount = 0.00f;
	} else {
		delaycount += 0.50f;
	}	
	
	if (animestate == [_textureAtlas count]) 
	{
...
-(bool) animateonce:(int) x: (int) y
{
	...
	for (id mykey in (_textureAtlas))
	{
		if (index == animestate)
		{
			[self drawFromAtlasAtPoint: CGPointMake(x, y) key: mykey];
			break;
		}
		index++;
	}
...
- (void) animate:(int) x: (int) y 
{
	...
	if (animestate == [_textureAtlas count]) animestate =0;
}
...
-(void) showframebykey: (NSString*) key: (int) x: (int) y
{
	...
	[self drawFromAtlasAtPoint: CGPointMake(x, y) key: key];
...
{
	return [_textureAtlas valueForKey:key];
}
